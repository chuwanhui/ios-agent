...
| **快捷指令工具** | 一个真实快捷指令 = 一个工具；设置页可**粘贴 JSON 批量导入**（参数支持类型 / 可选 / 枚举），也能让快捷指令末尾的**回调 URL 把结果传回来** |
...
3. 可选：
   ...
   - 设置 → 本地快捷指令工具：可以「从剪贴板粘贴」整段 JSON 导入，也可以手动填（参数用每行 `字段名=说明` 声明）
4. 在「快捷指令」App 里用「运行脚本」动作选中本脚本，即可接给 Siri 使用。

## 快捷指令工具：协议

一个真·快捷指令 = 一个工具。设置 → 本地快捷指令工具 → 从 JSON 导入：
...
- `name`：函数名（英文），模型看到的就是它；不填就按快捷指令名自动生成。
- `shortcut`：快捷指令名，要和「快捷指令」App 里**完全一致**。
...
也认 `{"tools":[…]}`、裸数组、`{"快捷指令名":{…}}` 映射，以及 `paramsHint` 那种一行一个 `字段名=说明` 的旧写法；导入是**追加**模式，同一个快捷指令已存在就跳过。「导出当前工具」可以把现有配置复制出来存档。
...
1. 「URL 编码」：输入 = 要回传的内容
2. 「文本」：写 `scripting://run/智能体?result=` 紧接着上一步的结果（中间不要有空格或换行）
3. 「打开 URL」：输入取上一步的文本
