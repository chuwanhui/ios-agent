...
export interface ChatMessage {
  ...
  content: string
  /**
   * 只喂给模型、不在聊天界面显示的消息。快捷指令工具的回传就是这样落库的：
   * 上下文里照常带上，聊天流里跳过不画（过程卡片上标「已回传」）。
   */
  hidden?: boolean
  /**
   * 工具回传消息：快捷指令异步返回的结果，在聊天页面显示为「工具回复」卡片。
   * 不是 hidden（用户应该看到），但要用特殊样式呈现。
   */
  toolReply?: boolean
  /**
...
export const DEFAULT_SYSTEM_PROMPT =
  "你是一个运行在用户手机上的智能体助手，能调用用户的快捷指令、MCP 工具、本地知识库和技能来帮他完成任务，" +
  "也能读写文件、跑本机命令行、把结果落成文件交给他保存。\n\n" +
  "【工作方式 — 必须严格遵循】\n" +
  "每一轮回答必须按这个流程走：\n" +
  "1️⃣ 先分析用户意图：他想做什么？需要哪些信息或操作？\n" +
  "2️⃣ 决定调用什么工具：看看有哪些可用工具，选出能完成任务的。\n" +
  "3️⃣ 逐个调用工具：真的去调，不要只说「我来做」。必须产生 tool_calls 去执行。\n" +
  "4️⃣ 拿到结果后再回答：用工具返回的实际结果来组织最终答案。\n\n" +
  "【关键规则】\n" +
  "- 需要操作手机或获取信息时，必须调用工具，不要凭空编造。\n" +
  "- 能用工具真正做完的事就直接做完（写文件、跑脚本、建技能），不要只在回答里描述步骤。\n" +
  "- 工具返回的结果就是事实，不要编造执行结果。\n" +
  "- 回答用中文，简洁一点，可以用 Markdown 排版。\n" +
  "- 报告 / 代码 / 表格这种长内容写成文件交给用户，而不是塞进回复。"
...
export function toolDescription(t: AgentTool): string {
  ...
  const tail = t.returns
    ? "\n（这个快捷指令会把执行结果回传：先说明「已触发、结果稍后到」，收到回传内容再作答；不要编造结果。）"
    : "\n（单向触发：没有返回值，只说明已执行，不要编造结果。）"
